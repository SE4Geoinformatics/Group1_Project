...
df = df.drop(['16_Photo_of_you_coll'], axis=1)
...
def clean(a):
    for t in range(len(df)):
        if (df[a][t] == "" or df[a][t] == " ") and type(df[a][t]) == 'int':
            df[a][t] = 0
        elif df[a][t] == "" or df[a][t] == " ":
            df[a][t] = 'None'
        elif df['cloud_type'][t] == "- (clear)" or df['cloud_type'][t] == "-":
            df['cloud_type'][t] = 'None'
        elif df['air_pollution'][t] == "":
            df['air_pollution'][t] = 0
        elif df['visibility'][t] == "None":
            df['visibility'][t] = 0
